...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (form) {
    form.addEventListener('submit', function(e) {
      ...
      emailjs.sendForm('service_ccbkuer', 'template_gk5zcen', this)
        .then(function() {
          // Send auto-reply to the user
          const formData = new FormData(form);
          emailjs.send('service_ccbkuer', 'template_gk5zcen', {
...
const PARTICLE_COLOR = "rgba(0,255,136,0.6)";  // neon green
const LINE_COLOR = "rgba(0,255,136,0.15)";     // line glow color
    ...
    const imageObserver = new IntersectionObserver((entries, observer) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          img.src = img.dataset.src;       // set the real src
          img.classList.remove('lazy-image');
          imageObserver.unobserve(img);    // stop watching
        }
      });
    }, {
      rootMargin: '100px 0px',             // start loading 100px before the image enters
      threshold: 0.01
    });
